 ...
 */
export const ActionTypes = { // task-item 组件，除了增，删，改，查功能， 还有移动一个item功能，移动整个tasklist功能，task完成功能
  ADD:                type('[Task] Add'),
...
export class MoveAction implements Action { // 移动task的时候，需要task本身的id ，和目标的tasklist的id
  type = ActionTypes.MOVE;

  constructor(public payload: { taskId: string; taskListId: string }) { }
}

export class MoveSuccessAction implements Action { // 成功后返回的还是一个task，不过tasklistid已经改变
...
export class MoveAllAction implements Action { // 移动整个task列表，需要原来的tasklistId，和目标的tasklistId
